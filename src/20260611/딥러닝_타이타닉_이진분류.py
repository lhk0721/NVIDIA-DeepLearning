...
# 4) 특성데이터의 스케일 변환(정규화) ==> 표준점수 정규화 ( 각특성 - 평균 / 표준편차 )
...
# 딥러닝 이진분류 모델 설계
model = Sequential()
model.add( Dense(8, input_dim=4, activation='leaky_relu')  )
...
model.fit(train_scaled, train_y ,batch_size=16, epochs=800, verbose=1)
